...
(function () {
  ...
  function findArticle() {
    ...
    const candidates = [
      ...
      '.page', '.article-content', '.article-body', '.post-content', '.entry-content',
  ...
  function loadVoices() {
    ...
    if (voices.length && !selectedVoice) {
      const configPref = cfg.defaultVoice ? voices.find(v => v.name.toLowerCase().includes(cfg.defaultVoice.toLowerCase())) : null;
      const autoPref = voices.find(v => v.name.includes('Natural') || v.name.includes('Enhanced') || v.name.includes('Google'));
      selectedVoice = (configPref || autoPref || voices[0]).name;
    }
  ...
  function init() {
    ...
    if (cfg.position === 'float') {
      ...
      articleEl.insertBefore(widgetEl, articleEl.firstChild);
    }
